...
const AddtoCartMobileview = ({ salePrice, filterPrice, proQty, setProQty, addedToCart }: { addedToCart: any, salePrice: any, filterPrice: any, proQty: any, setProQty: any }) => {
    ...
    return (
        <div className="fixed sm:bottom-[3.5rem]  bottom-[4rem] px-[5px] left-0 right-0 md:hidden border-t border-gray-300   backdrop-blur-sm bg-opacity-95 bg-slate-100  h-16 py-1 items-center z-30">
            <div className="flex justify-between overflow-x-auto no-scrollbar space-x-2 translate-y-1/4">
                ...
                <div className="flex h-8 mt-auto max-[300px]:hidden">
                    ...
                    <Button className="!px-1 h-[28px] w-[28px] rounded-none" onClick={() => proQty >= 1 ? setProQty((preQty: number) => preQty + 1) : null}>
